...
//
// Everything that drives the real engine goes through `ayq-engine-harness.ts`;
// this file is the questions. Build 005's own subjects — balances and anchors,
// owner counterparty names, the starter taxonomy and the Plan's historical
// suggestions — live in `ayq-build-005.test.ts`, because each of their tests
// creates a real Actual budget and the runner's timeout is per file.
//
...
// is creating real Actual budgets, and on the Windows runner that is disk
// rather than processor: running two at once was tried and made the file
// slower — 184s to 317s — because two Electron processes writing SQLite
// contend for a disk that scans everything written to it.
//
// That cost is also why this file has a sibling. Run 73 put build 005's tests
// in here as well and the file went past the runner's per-file timeout with
// every assertion in it passing: 188 passed, none failed, the file cancelled.
// The limit is per file, so the answer to a suite that keeps growing is another
// file — not a bigger number on its own.
...
test('a ledger that does not match states the difference, and changes nothing', async () => {
  ...
  assert.ok(coverage.ledgerBalanceCents !== null);
  assert.equal(
  ...
  // The balance is the bank's own figure at the anchor point, and the
  // difference above is the *movements* not accounting for it (§4.2 read with
  // 03 §8). Build 004 compared the bank's figure with the balance and could
  // only ever have found them equal once §4.2 existed, which is why the
  // comparison moved to the movements and this assertion moved with it.
  const accounts = await ask(dataDir, { kind: 'accounts.list' });
  assert.ok(accounts[0].balanceCents !== null);
  assert.ok(beforeAccounts[0].balanceCents !== null);
  assert.equal(
    accounts[0].anchor?.source,
    'bank',
    'the bank stated a closing balance and it became the anchor',
  );
  assert.equal(accounts[0].anchor?.amountCents, 200_000);
  assert.notEqual(
    accounts[0].balanceCents,
    coverage.ledgerBalanceCents,
    'what AYQ can account for is not the same as what the account holds',
  );

  // §8.5: derived on read. Asking again gives the same answer and writes
  // nothing — the store holds the evidence, and no reconciliation decision.
  ...
  assert.ok(store.evidence, 'the statements AYQ has read are kept');
  assert.equal(
...
test('a version 3 store is carried forward whole, with §7.14 s dates filled in', async () => {
  ...
  // The store on disk has been brought forward, and the copy 03 §5.3 requires
  // is beside it. Build 004 could leave the file at its old version until
  // something happened to write; build 005 cannot, because opening a budget
  // provisions the starter taxonomy and writes its marker (11 §11.3). That is
  // a write on open, so the upgrade lands on open — which is safe precisely
  // because the copy is made first, and is checked here rather than assumed.
  const onRead = JSON.parse(
    await readFile(join(dataDir, 'ayq-store.json'), 'utf8'),
  ) as { version: number };
  assert.equal(onRead.version, AYQ_STORE_VERSION);
  assert.ok(
    existsSync(join(dataDir, `ayq-store.before-v3-to-v${AYQ_STORE_VERSION}.json`)),
    'the store was brought forward without a copy being kept first',
  );
...
test('spending answers by category, and counts what nobody filed', async () => {
  ...
  const categories = await ask(dataDir, { kind: 'categories.list' });
  const transport = categories.find(category => category.name === 'Public transport');
  assert.ok(transport);
  const ledger = await ask(dataDir, { kind: 'transactions.list' });
  const fuel = ledger.rows.find(row => row.payee === 'Testfuel');
  assert.ok(fuel);
...
test('categories survive a restart, and a re-import adds nothing', async () => {
  ...
  assert.ok(
    reopened.rows.every(row => row.category === 'Public transport'),
  ...
  assert.ok(afterReimport.rows.every(row => row.category === 'Public transport'));
});
...
test('an alias does not overwrite a category a person filed by hand', async () => {
  ...
  const filed = after.rows.filter(row => row.category === 'Public transport');
  ...
  assert.equal(rules[0].categoryName, 'Public transport');
});
...
test('a record can be edited, dismissed, accepted back and removed', async () => {
  ...
  assert.equal(edited.records[0].categoryName, 'Health & pharmacy');
...
test('the flag is the owner’s, per account, and it survives a restart', async () => {
  ...
  const savings = both.find(one => (one.balanceCents ?? 0) > 700_000);
...
test('the forecast starts from available funds and takes the plan and the records', async () => {
  ...
  const accounts = await ask(dataDir, { kind: 'accounts.list' });
  const savings = accounts.find(one => (one.balanceCents ?? 0) > 700_000);
  assert.ok(savings);
  await ask(dataDir, {
    kind: 'accounts.setFlag',
    accountId: savings.id,
