...
//
// All or nothing (02 §7.8–§7.16, 03 §13.10): the file is written beside its
// destination under one fixed partial name, read back and validated there, and
// only then moved over the destination. An export that is interrupted or
// refused at any point before that move leaves the previous snapshot exactly
// as it was — it is never replaced by anything that did not validate.
...
import { ayqBudgetMonths } from './ayq-budget.ts';
...
import { ayqReadStore, ayqRowKey, ayqStandingDecision } from './ayq-store.ts';
...
export async function ayqBuildAnalyticalSnapshot(
  ...
  for (const summary of summaries) {
    ...
    if (identifier === null) {
      throw new AyqEngineError(
        ...
        `account "${summary.name}" carries no masked identifier AYQ can vouch for`,
      );
    }
  ...
  // ---- category plans ------------------------------------------------------
  // The plan is Actual's (02 §5.1): what Plan shows for a category in a month.
  // A month with nothing planned for a category is not a plan of nought; it
  // does not cross.
  const categoryPlans: CategoryPlan[] = [];
  for (const month of await ayqBudgetMonths()) {
    const kept = (await api.getBudgetMonth(month)) as unknown as {
      categoryGroups?: Array<{ categories?: Array<{ id: string; budgeted?: number }> }>;
    };
    for (const group of kept.categoryGroups ?? []) {
      for (const category of group.categories ?? []) {
        const planned = Number(category.budgeted ?? 0);
        const categoryId = categoryIdByActual.get(category.id);
        if (planned === 0 || categoryId === undefined) continue;
        categoryPlans.push({ categoryId, month, plannedAmount: money(planned) });
      }
    }
  }
  ...
  for (const occurrence of plan.occurrences) {
    ...
    } else if (occurrence.state === 'dismissed') {
      state = 'dismissed';
    } else if (occurrence.state === 'overdue' && occurrence.effectiveDate < today) {
      state = 'overdue';
    } else {
      state = 'expected';
    }
  ...
  return validated(JSON.parse(JSON.stringify(snapshot)));
}

/** The contract's validator, its refusal carried as AYQ's own code. */
function validated(value: unknown): AnalyticalSnapshotV1 {
  try {
    return validateAnalyticalSnapshot(value);
  } catch (error) {
    throw new AyqEngineError(
      'snapshot-invalid',
      `the snapshot does not validate against contract ${CONTRACT_VERSION}: ${
...
/** The one partial name an export writes before it is proven. */
export function ayqSnapshotPartialPath(path: string): string {
  return `${path}.ayq-partial`;
}

/**
 * Writes `text` to `path`, all or nothing.
 *
 * The bytes go to the partial file beside the destination; they are read back
 * from disk, compared byte for byte and validated by the contract; only then is
 * the partial moved over the destination, in one rename. Until that rename the
 * destination is not touched, so whatever stops the export first — a failed
 * write, a full disk, a refusal, the process ending — leaves the previous
 * snapshot as it was. A partial left by an export that never finished is
 * written over by the next one; it is never read as a snapshot.
 *
 * `interrupt` stands in for the process ending between the proof and the
 * move, for the test that shows the previous snapshot survives it.
 */
export function ayqWriteSnapshotFile(
  path: string,
  text: string,
  interrupt?: () => void,
): number {
  const bytes = Buffer.byteLength(text, 'utf8');
  const partial = ayqSnapshotPartialPath(path);
  const removePartial = (): void => {
    // Only a file of AYQ's own is taken away; anything else at that name is
    // not AYQ's to remove, and the failure says so.
    try {
      if (lstatSync(partial).isFile()) rmSync(partial, { force: true });
    } catch {
      // Nothing there.
    }
  };
  try {
    ...
    const back = readFileSync(partial);
    if (back.length !== bytes || back.toString('utf8') !== text) {
      throw new Error(`the partial file reads back as ${back.length} bytes where ${bytes} were written`);
    }
    validated(JSON.parse(back.toString('utf8')));
    interrupt?.();
    renameSync(partial, path);
  } catch (error) {
    removePartial();
    if (error instanceof AyqEngineError) throw error;
    throw new AyqEngineError(
  ...
  // Success is claimed only for what can be shown: the file is at the path the
  // owner chose, at the size that was written, and no partial is left.
  let written: number;
  try {
    written = statSync(path).size;
  } catch {
    throw new AyqEngineError('snapshot-write-failed', `the file was not there after writing it: ${path}`);
  }
  if (written !== bytes) {
    throw new AyqEngineError(
      'snapshot-write-failed',
      `the file on disk is ${written} bytes where ${bytes} were written: ${path}`,
    );
  }
  if (existsSync(partial)) {
    throw new AyqEngineError('snapshot-write-failed', `the partial file was not moved into place: ${partial}`);
  }
  return bytes;
}

/**
 * Builds the snapshot and writes it to `path` (see ayqWriteSnapshotFile).
 * Returns counts and the location only — nothing in the answer is a figure
...
export async function ayqExportAnalyticalSnapshot(
  ...
  about: AyqAbout,
  path: string,
): Promise<AyqSnapshotWritten> {
  const snapshot = await ayqBuildAnalyticalSnapshot(dataDir, budgetId, today, about);
  const bytes = ayqWriteSnapshotFile(path, JSON.stringify(snapshot, null, 2));
  return {
    outcome: 'written',
    path,
